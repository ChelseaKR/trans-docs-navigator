...
type Label = {
  ...
  unverified: number;
  /** Mechanical corpus/roster result only; it is not counsel or launch approval. */
  mechanical_verification_complete: boolean;
  /** Never auto-granted by a build script; human launch gates remain authoritative. */
...
for (const rec of records) {
  ...
  if (
    ...
    isPlaceholderVerifier(rec.source.verifier, roster)
  ) {
    l.mechanical_verification_complete = false;
  }
}
...
const incomplete = Object.values(labels).filter(
  (label) => !label.mechanical_verification_complete,
).length;
pass(
  "dataset-build",
  `wrote ${records.length} record(s) across ${Object.keys(labels).length} jurisdiction(s) to ` +
    `${outDir.replace(REPO_ROOT + "/", "")} (${incomplete} jurisdiction(s) mechanically incomplete; ` +
    `launch clearance remains human-gated — see labels.json)`,
);
