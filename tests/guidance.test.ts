...
test("answer refuses when no current record backs the request (no fabrication)", () => {
  // Puerto Rico court-order: no territory record, no federal court-order → genuine refusal.
  const ans = answer({ jurisdiction: "US-PR", change_types: ["name"], documents: ["court-order"], today });
...
test("unsupported state still gets valid federal guidance (SSA/passport)", () => {
  const ans = answer({ jurisdiction: "US-PR", change_types: ["name"], today });
  assert.equal(ans.refused, false);
