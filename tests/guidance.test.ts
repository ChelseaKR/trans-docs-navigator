...
test("answer refuses when no current record backs the request (no fabrication)", () => {
  // Alabama court-order: no state record, no federal court-order → genuine refusal.
  const ans = answer({ jurisdiction: "US-AL", change_types: ["name"], documents: ["court-order"], today });
...
test("unsupported state still gets valid federal guidance (SSA/passport)", () => {
  const ans = answer({ jurisdiction: "US-AL", change_types: ["name"], today });
  assert.equal(ans.refused, false);
