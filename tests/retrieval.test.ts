...
// PR's US-AL/MS/LA addition — these two fixtures have swapped states before as each
// gained corpus coverage.
test("unsupported state still surfaces federal records, but no state records", () => {
  const r = retrieve({ jurisdiction: "US-AK", change_types: ["name"], today });
  assert.ok(r.length > 0);
  assert.ok(r.every((x) => x.record.jurisdiction === "US")); // federal only; no Alaska records exist
});
...
test("unsupported state with a state-only document (court-order) returns nothing", () => {
  ...
  const r = retrieve({ jurisdiction: "US-AK", change_types: ["name"], documents: ["court-order"], today });
