...
> **Provenance note (added on merge, 2026-08-04).** This document was written
> 2026-06-30 on a local branch (`research-panel-and-roadmap`) that was never pushed
> until PR #104 salvaged it for preservation. That branch also contained code changes
> for several items below; **those code changes were not merged.** By the time of
> salvage, `main` had independently implemented the same intent for at least R11 (see
> `src/render.ts`'s "report an error / law changed" link, which cites this roadmap's
> item ID directly) and E6 (see `public/assets/reminders.js`, a later and more
> refined pass — RFC 5545 line-folding and de-branded output — than this branch's
> version). Items below marked **"✅ Implemented 2026-06-30 (working tree,
> uncommitted)"** describe that branch's now-superseded code, not necessarily what
> ships today. Before treating any item here as open or closed, check current `main`
> rather than trusting this doc's status markers.
>
