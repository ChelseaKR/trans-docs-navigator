...
| Save for offline / delete offline copies | checklist, packet, offline-notice pages | `Tab` to Save/Delete buttons | `#offline-status` is `role="status" aria-live="polite"`; confirm the "no browser support" disabled-button state is itself announced, not just visually grayed out |
