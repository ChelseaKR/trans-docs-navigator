...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 334** records verified by a named human. 334 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 497 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **179 UNCHECKABLE**. Separately, **1023 of 1270 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
