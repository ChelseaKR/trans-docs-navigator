...
| **M1 — Corpus & data model** | ✅ Done (seed) | 84 schema-validated records (6 states + federal; 42 EN + 42 ES); `make content` + `make freshness` + `make fidelity` green. Content is **seed data**, not launch-verified (ADR-3) — the record/verifier counts in the generated launch-gate table below are the machine-derived source of truth. |
...
| **M6 — Expand jurisdictions** | ◑ In progress | 6 states + federal represented (TX + WA + OH added through the gates). Each carries a **mechanical readiness** row in `eval-report.md`; **launch-clearance is review-gated and OPEN** for all. TX DMV gender-marker and OH's driver's-license/birth-certificate gender-marker records are `needs_reverification` (volatile, actively-restricted areas) — demonstrates per-jurisdiction degradation. Ohio's birth-certificate and driver's-license/ID sex-marker paths are stated as contested/unavailable rather than described as working processes (ODH's own page is unfetchable under this project's declared user-agent — same class as `health.ny.gov` — so it is human-read but machine-uncheckable). |
...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 109** records verified by a named human. 109 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 172 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **33 UNCHECKABLE**. Separately, **313 of 386 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
| Every cited source actually under drift watch | 🔴 **OPEN** | **4** cited source(s) are UNWATCHABLE — they refuse this project's declared user-agent (403), so no baseline can be taken or compared and drift there is undetectable: `https://www.nycourts.gov/courthelp/Family/nameChange.shtml`, `https://odh.ohio.gov/know-our-programs/vital-statistics/changing-correcting-birth-record`, `https://www.ssa.gov/forms/ss-5.pdf`, `https://www.health.ny.gov/vital_records/gender_designation_corrections.htm` | `corpus/source-hashes.json` + `forms/form-hashes.json` + `corpus/snapshots/index.json` |
