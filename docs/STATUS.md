...
| **M6 — Expand jurisdictions** | ◑ In progress | 6 states + federal represented (TX, WA, and MA added through the gates). Each carries a **mechanical readiness** row in `eval-report.md`; **launch-clearance is review-gated and OPEN** for all. TX DMV gender-marker is `needs_reverification` (volatile) — demonstrates per-jurisdiction degradation. Massachusetts vital records is another: its own source says a birth certificate cannot be amended for a legal name change alone, so `ma.birth-certificate.name` states that restriction plainly instead of implying a general route. |
...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 197 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **40 UNCHECKABLE**. Separately, **304 of 382 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
