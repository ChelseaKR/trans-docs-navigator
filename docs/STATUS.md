...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 980** records verified by a named human. 980 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 1033 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **498 UNCHECKABLE**. Separately, **2546 of 3122 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
