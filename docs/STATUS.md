...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 909** records verified by a named human. 909 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 997 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **510 UNCHECKABLE**. Separately, **2507 of 3070 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
