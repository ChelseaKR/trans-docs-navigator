...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 889** records verified by a named human. 889 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 991 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **476 UNCHECKABLE**. Separately, **2421 of 2969 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
