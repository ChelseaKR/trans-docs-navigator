...
> + rate limiting (§2.2), a runtime PII-egress data-flow test (§2.1, which caught and
...
> (§2.4), non-PII observability counters (§6.2), and a contribution path (§7.2). The
...
| **M4 — Client-side form pre-fill** | ✅ Done | `api/forms.ts` + `src/pages.ts` (browser `pdf-lib`); fillable forms filled client-side, flat scans degrade to download. No PII to server. |
| **M5 — Experience & a11y hardening** | ✅ Done (auto-gated parts) | Full flow + ephemeral-by-default (explicit "private mode" affordance) + **printable packet** (`/packet`, print CSS, no-JS-friendly) + **Spanish parity** (13 ES records, EN/ES both 100% on the gold set) + keyboard-path tests; mechanical a11y auto-gated across 7 templates. Manual SR/keyboard/zoom walkthrough remains **review-gated (PENDING)**. |
...
| #3 Privacy is a safety property | zero server-side PII; client-side fill; allowlist logger; `make privacy` | ✅ auto |
...
- [ ] DPIA + STRIDE threat-model sign-off. *Enforced:* privacy lint + runtime PII-egress test.
