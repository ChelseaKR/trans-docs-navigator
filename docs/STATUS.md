...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 372 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **110 UNCHECKABLE**. Separately, **636 of 802 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
