...
| **M5 — Experience & a11y hardening** | ✅ Done (auto-gated parts) | Full flow + no saved browser session by default (explicit "private mode" affordance) + **printable packet** (`/packet`, print CSS, no-JS-friendly) + **Spanish parity** (301 ES records, EN/ES both 100% on the gold set) + keyboard-path tests; mechanical a11y auto-gated across 25 templates. Manual SR/keyboard/zoom walkthrough remains **review-gated (PENDING)**. |
