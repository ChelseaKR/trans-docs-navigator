...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 928** records verified by a named human. 928 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 1015 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **490 UNCHECKABLE**. Separately, **2489 of 3053 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
