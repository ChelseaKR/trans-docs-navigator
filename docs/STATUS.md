...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 169 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **54 UNCHECKABLE**. Separately, **289 of 364 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
| Every cited source actually under drift watch | 🔴 **OPEN** | **4** cited source(s) are UNWATCHABLE — they refuse this project's declared user-agent (403), so no baseline can be taken or compared and drift there is undetectable: `https://www.nycourts.gov/courthelp/Family/nameChange.shtml`, `https://www.pa.gov/content/dam/copapwp-pagov/en/health/documents/topics/documents/certificates-and-records/Request%20to%20Modify%20an%20Adult's%20Birth%20Record.pdf`, `https://www.ssa.gov/forms/ss-5.pdf`, `https://www.health.ny.gov/vital_records/gender_designation_corrections.htm` | `corpus/source-hashes.json` + `forms/form-hashes.json` + `corpus/snapshots/index.json` |
