...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 239** records verified by a named human. 239 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 381 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **131 UNCHECKABLE**. Separately, **700 of 884 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
