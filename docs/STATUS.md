...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 319** records verified by a named human. 319 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 478 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **175 UNCHECKABLE**. Separately, **994 of 1232 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
