...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 359 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **123 UNCHECKABLE**. Separately, **643 of 816 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
