...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 226** records verified by a named human. 226 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 373 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **110 UNCHECKABLE**. Separately, **647 of 808 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
