...
| **M6 — Expand jurisdictions** | ◑ In progress | 6 states + federal represented (TX, WA, and AZ added through the gates). Each carries a **mechanical readiness** row in `eval-report.md`; **launch-clearance is review-gated and OPEN** for all. TX DMV gender-marker and both AZ gender-marker records are `needs_reverification` (volatile/litigated) — demonstrates per-jurisdiction degradation. |
...
|---|---|---|---|
| Named-human verification of every record | 🔴 **OPEN** | **0 of 110** records verified by a named human. 110 carry the `Pilot Seed Reviewer` placeholder. | `corpus/` + `forms/registry.json` × `corpus/VERIFIERS.json` |
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 180 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **36 UNCHECKABLE**. Separately, **310 of 388 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
| Every cited source actually under drift watch | 🔴 **OPEN** | **5** cited source(s) are UNWATCHABLE — they refuse this project's declared user-agent (403), so no baseline can be taken or compared and drift there is undetectable: `https://superiorcourt.maricopa.gov/media/emucljue/name-gender-change-eng-spa.pdf`, `https://www.nycourts.gov/courthelp/Family/nameChange.shtml`, `https://www.ssa.gov/forms/ss-5.pdf`, `https://www.azdhs.gov/documents/vital-records/manuals/correction-affidavit-correct-amend-birth.pdf?v=20260409`, `https://www.health.ny.gov/vital_records/gender_designation_corrections.htm` | `corpus/source-hashes.json` + `forms/form-hashes.json` + `corpus/snapshots/index.json` |
