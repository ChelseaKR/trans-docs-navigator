...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 375 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **114 UNCHECKABLE**. Separately, **664 of 832 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
