...
| Every record's claims backed by its own cited source | 🔴 **OPEN** | 369 assertion(s) located in their cited source, 0 unsupported (merge-blocking), **128 UNCHECKABLE**. Separately, **661 of 828 prose sentences carry no checkable literal** and no gate vouches for them. | `make fidelity` (`scripts/source-fidelity.ts`) |
