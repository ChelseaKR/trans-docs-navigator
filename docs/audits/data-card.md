...
California, Florida, Illinois, New York, Texas, Washington. English coverage spans
all six states + federal, and **Spanish coverage now matches it record-for-record**
(40 EN + 40 ES) — every state that has an EN record has its ES twin.

**Birth certificates (34 records, EN + ES).** Every state's own vital-records/health-
department source, including the `court-order → birth-certificate` prerequisite edge
each state states in its own words, and the closed routes in Texas and Florida: Texas's
Vital Statistics page lists only hospital records or a hospital clerical-error letter as
evidence for the sex field, and Florida's Department of Health amendments page lists
name changes, paternity, and adoption corrections but no route to change the sex field
to match gender identity — neither cites a way to change sex to match gender identity.
That "not available here" is a **cited, served claim**, not a silence — see
`docs/RELOCATION.md` for why the planner treats a birth certificate as governed by the
state of *birth*, which a move cannot change.

**Florida (added most recently).** Florida restricted both its driver-license/ID
gender-marker route and its birth-certificate sex-designation route around 2024; no
official Florida government page currently states a working process for either, so
those two records say so plainly (citing the department's own page that lists what it
*does* offer) rather than describing a route that no longer works, and both carry a
shortened 30-day re-check SLA rather than the usual 90.
...
- Spanish now covers every state EN does (6 states + federal), record-for-record.
- Federal gender-marker policy (SSA, passport) and the **Texas** DMV sex-marker policy are
  ...
  **Florida's** driver-license and birth-certificate gender-marker records instead state
  plainly that the cited official page describes no route, on a shortened 30-day SLA — a
  different, source-honest way of surfacing the same volatility (see PR #119's house
  standard on not rendering an unavailable route as a working plan).
