...
| 2026-09-05 | Minors pilot (`Intake.for_minor`, `RelocationIntake.for_minor`): one new selection-only bookkeeping bit, same privacy class as `has_court_order` — no new identity field. Rows updated for checklist selections and save/resume | Mechanically derived from code; not a re-verification |
| 2026-09-17 | Google Analytics 4 page counts (ADR 0007, owner decision): inventory row, control row and reviewer question added. The Privacy Notice's "No cookies, no trackers" section is replaced by a GA disclosure | Owner-directed change; derived from `public/assets/analytics.js` and `tests/analytics.test.ts`; not a human DPIA re-verification |
...
| Checklist selections (state, change types, documents, language, has-court-order bookkeeping bit, for-minor bookkeeping bit — minors pilot, `Intake.for_minor`) | Browser memory/history + GET URL; transient server processing; selection-only checklist/answer output and canonical selection key may enter the bounded process-memory render cache (`api/cache.ts`: hard size cap, oldest-first eviction, process-local, cleared on corpus change); allowlisted fields enter application logs | Cache: until eviction or process restart. Application logs: 14 days in the live preview (`infra/preview/main.tf`); 30 days in the production template (`infra/main.tf`). Browser/edge-provider retention is independent | Application process/operators; hosting or edge providers may see request/network metadata |
| **Packet staleness check (`since` plus the checklist selections — GET `/changes`, `api/changes.ts`)**. `since` is the date a specific person printed a specific packet. Joined with a jurisdiction it narrows a person further than any other selection on this route, so it gets the relocation posture rather than the checklist one | Browser memory/history + GET URL; transient server processing only. **Not memoized** — a memo key keyed on `since` would retain "this person printed a packet on this date" in process memory for the process lifetime, the same reasoning `/plan` applies. **Nothing derived from `since` reaches the log**: not the date, and not the one-bit "is this packet past the longest recheck window", which would still narrow a person to a range of print dates. The route's log descriptor carries exactly what `/packet`'s does (jurisdiction, language, status). `noindex` + `robots.txt` `Disallow: /changes` | No application persistence, no cache entry. The GET URL may persist in browser history and upstream proxy/edge/provider access records under their own retention | Application process/operators; hosting or edge providers may see request/network metadata. The printed packet carries this URL on paper, so the date also lives wherever the paper does |
...
| **Google Analytics 4 page counts (ADR 0007, hosted preview only)** — `public/assets/analytics.js` on every rendered page | Google LLC (US), GA4 property 554877190. Receives: origin + **path only** (no query string, so no selection, court-order/minor bit, `since` date or question; no fragment), page title, referring **origin** only, browser/device/screen/language, approximate location Google derives from the IP address (GA4 does not store IP addresses), and enhanced-measurement events the stream enables (scroll, outbound click). Never loads: off the production host, under GPC or Do Not Track, after the footer opt-out (localStorage `trans-docs-navigator:analytics-opt-out`), on any page whose URL carries `q`/`s`/`search`/`query`/`keyword`, or on `/move` and `/plan` | 14 months in the property; `_ga`/`_ga_8HZFD5R23E` cookies up to two years outside the EEA/UK/CH, none inside them (analytics storage denied; cookieless pings) | Google, and the repository owner through the GA4 property. Ad signals denied, Google signals and ad personalization off |
...
| Google Analytics receives no selection, question, identity field or relocation visit | **Shipped, code-enforced:** `tests/analytics.test.ts` runs the loader and proves `page_location` is origin + path, that `q`/site-search pages and `/move`/`/plan` load nothing, that GPC/DNT/opt-out load nothing, and that each guard's removal is caught (negative controls). `api/csp.ts` admits only the minimum GA origins |
...
- **[Reviewer question, added 2026-09-17] Google Analytics for this population.** GA4
  page counts (ADR 0007) are path-only and skip question pages and the relocation planner,
  but Google still learns that a browser (pseudonymous `_ga` cookie outside the EEA/UK/CH,
  approximate location from its IP address) viewed, for example, `/guide/US-TX` or
  `/checklist` at a given time. Is a page-count path to a US company acceptable for users
  in hostile jurisdictions, given GPC/DNT and the opt-out? The owner accepted it on
  2026-09-17; the named reviewer has not.
