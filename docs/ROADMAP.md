...
| Metrics (`/metrics`, RED per endpoint, UCUM naming) | **Live (in-process seam)** | `api/metrics.ts` — Prometheus text-exposition `/metrics`, `requests_total`/`errors_total`/`duration_seconds` histogram per BOUNDED route template (never a raw path); `tests/metrics.test.ts`. This is the deterministic in-process default (ADR-1/ADR-2 pattern); wiring an OTel `PeriodicExportingMetricReader` → Collector → Prometheus/Mimir pipeline for a real deployment is a drop-in swap behind the same `recordRequest()` call site and remains open |
