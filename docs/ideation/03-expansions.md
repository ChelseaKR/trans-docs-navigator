...
- **Status:** Shipped. `scripts/dataset-build.ts` assembles a release bundle
  ...
  flag (`launch_cleared:false` wherever a placeholder verifier is involved) — no new
  legal wording, same framing as `docs/audits/data-card.md`. `scripts/dataset-diff.ts`
  ...
  `dataset` job runs the build on the same `tags: v*` trigger, uploads the bundle as
  a workflow artifact, and attests keyless build provenance via
  ...
  membership, and the placeholder → `launch_cleared:false` invariant. Zenodo DOI
  minting is still open (noted above) — this ships the build/sign/diff pipeline the
  DOI step would attach to.
