...
| Documentation | ✅ | `CITATION.cff`, `SECURITY.md` (private vuln reporting), `CHANGELOG.md`, `.standards-version`, currency stamps on every audit artifact, and this table. ⚠️ Open: `docs/adr/` contains only the seed record — inline ADRs not yet migrated (see Code Quality row). |
